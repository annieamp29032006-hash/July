...
// 1. Insert activeBauCuaSessions at top level
...
// 2. We will find exactly where handleBauCuaUI is, and where doBauCuaAnimation is.
const oldHandleBauCuaUI = code.match(/async function handleBauCuaUI\([\s\S]*?async function doBauCuaAnimation/);
if (oldHandleBauCuaUI) {
    const newHandleBauCuaUI = `async function handleBauCuaUI(ctx, isSlash) {
...
async function doBauCuaAnimation`;
    code = code.replace(oldHandleBauCuaUI[0], newHandleBauCuaUI);
}

const oldDoBauCuaAnimation = code.match(/async function doBauCuaAnimation[\s\S]*?async function doChanLeAnimation/);
if (oldDoBauCuaAnimation) {
    const newEndBauCuaSession = `async function updateBauCuaUI(session) {
...
async function doChanLeAnimation`;
    code = code.replace(oldDoBauCuaAnimation[0], newEndBauCuaSession);
}

// Fix the button / modal placement by REMOVING them from the bottom and inserting them ABOVE `if (!interaction.isChatInputCommand()) return;`

// 1. Remove old button block
const btnBlock = code.match(/  if \(interaction\.isButton\(\) && interaction\.customId\.startsWith\('baucua_btn_'\)\) \{[\s\S]*?  \}/);
if (btnBlock) {
    code = code.replace(btnBlock[0], '');
}

// 2. Remove old modal block
const modalBlock = code.match(/  if \(interaction\.isModalSubmit\(\) && interaction\.customId\.startsWith\('baucua_modal_'\)\) \{[\s\S]*?  \}/);
if (modalBlock) {
    code = code.replace(modalBlock[0], '');
}

// 3. Create the new combined block
const newBlocks = `
  ...
  if (interaction.isModalSubmit() && interaction.customId.startsWith('baucua_modal_')) {
      ...
      return interaction.reply({ content: \`✅ Bạn đã cược thành công **\${bet.amount.toLocaleString('en-US')} MLZ Coin** vào **\${BAUCUA_NAMES[choiceId]}**!\`, ephemeral: true });
  }

  if (!interaction.isChatInputCommand()) return;`;

// 4. Replace `if (!interaction.isChatInputCommand()) return;` with the new blocks
code = code.replace('  if (!interaction.isChatInputCommand()) return;', newBlocks);
